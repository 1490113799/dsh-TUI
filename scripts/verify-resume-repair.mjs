#!/usr/bin/env node
/**
 * Regression: pre-resume session-log repair
 ...
 *   1. unknown types get ignorable:true (the resume-blocking case — this
 *      process never loads working-activity, so its #119 registration never
 *      runs and activity/status stays unknown, mirroring a plugin-less
 *      consumer);
 *   2. known types are never marked (probe coherence: the repair's probe IS
 *      the known-types list, so plugin-registered types pass unmarked);
 ...
 *   6. frame 0 keeps its exact original bytes (the header-frame invariant
 *      the backend's assertZstdHeaderFrame depends on).
...
const { repairSessionLogForResume } = await import('../lib/types/dsh-adapter/compat/sessionLog.js')
...
const frame0 = zstdCompressSync(Buffer.from(JSON.stringify(header) + '\n', 'utf8'))
writeFileSync(
  file,
  Buffer.concat([
    frame0,
    ...frames.slice(1).map((f) => zstdCompressSync(Buffer.from(f.map((e) => JSON.stringify(e)).join('\n') + '\n', 'utf8'))),
  ]),
)

const decodeAll = () => {
  // Per-frame decode (magic scan): a single whole-file zstdDecompressSync
  // yields only the first frame on newer Node, not the concatenation.
  const buf = readFileSync(file)
  const offsets = []
  for (let i = 0; i + 4 <= buf.length; i++) {
    if (buf.readUInt32LE(i) === 0xfd2fb528) offsets.push(i)
  }
  return offsets
    .map((start, i) => zstdDecompressSync(buf.subarray(start, i + 1 < offsets.length ? offsets[i + 1] : buf.length)).toString('utf8'))
    .join('')
    .split('\n')
    .filter(Boolean)
    .map((l) => JSON.parse(l))
...
// Frame-0 byte invariant: the header frame was not dirty, so it must have
// been copied verbatim — listings read only that frame.
const repairedBytes = readFileSync(file)
assert.equal(
  Buffer.compare(frame0, repairedBytes.subarray(0, frame0.length)),
  0,
  'frame 0 copied verbatim',
)
