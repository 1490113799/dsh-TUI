...
const dict = {
    ...
    'mcp-insert-hint': { zh: '在 profile 补丁层（~/.dsh/profiles/cc-tui/cordis.patch.yml）insert 一行即可，例：', en: 'Insert one line in the profile patch layer (~/.dsh/profiles/cc-tui/cordis.patch.yml), e.g.:' },
    ...
    'export-title': { zh: '# dsh-cc 会话导出', en: '# dsh-cc session export' },
    ...
    'doctor-launch-hint': { zh: '启动方式  dsh-cc.cmd / dsh --config <上述任一配置>', en: 'Launch      dsh-cc.cmd / dsh --config <either config above>' },
    ...
    'login-logout-hint': { zh: 'DSH 凭证来自环境变量 DEEPSEEK_API_KEY — 删除该环境变量后重启 dsh-cc 即登出', en: 'DSH credentials come from the DEEPSEEK_API_KEY env var — remove it and restart dsh-cc to log out' },
    ...
    'permissions-approval-hint': { zh: 'DSH 的 /permission 预设切换需要 approval 服务 + 审批 UI，dsh-cc 未挂载。', en: 'DSH /permission preset switching needs the approval service + approval UI, not mounted in dsh-cc.' },
    ...
    'update-unavailable': { zh: '当前运行方式不支持自动更新（需经 dsh --profile 启动），请在终端执行 dsh plugin --profile <name> update dsh-cc-tui', en: 'Automatic update is unavailable in this launch mode (needs dsh --profile). Run dsh plugin --profile <name> update dsh-cc-tui in a terminal.' },
    ...
    'update-starting': { zh: '正在更新 dsh-cc-tui，完成后会自动重启并恢复当前会话……', en: 'Updating dsh-cc-tui. The TUI will restart and resume this session when finished…' },
