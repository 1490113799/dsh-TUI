...
export function Chat({ channel, questionStore, approvalStore, onExit, onUpdate, fullscreen = false, trajectorySeen: trajectorySeenProp, }) {
    ...
    trajectoryRef.current = extendTrajectory(trajectoryRef.current, 
    ...
    const failureHintRowId = React.useMemo(() => {
        ...
        for (let index = channel.rows.length - 1; index >= 0; index--) {
            ...
            if (row?.kind === 'tool' && row.tool?.status === 'error')
                return row.id;
        }
        return null;
        // oxlint-disable-next-line react-hooks/exhaustive-deps
    ...
    if (sceneOpen) {
        const scene = _jsx(TrajectoryScene, { channel: channel, build: trajectory, onClose: closeScene });
    ...
    return (_jsxs(Box, { ref: wakeTickRef, flexDirection: "column", flexGrow: 1, width: "100%", children: [!isSticky && channel.lastUserText && (_jsx(StickyPromptHeader, { text: channel.lastUserText, onClick: () => {
                    ...
                    if (lastUser)
                        ...
                        handle?.scrollToBottom();
                } })), _jsxs(ScrollBox, { ref: setHandle, flexDirection: "column", flexGrow: 1, flexShrink: 1, stickyScroll: true, children: [_jsx(LogoHeader, { model: channel.model, effort: channel.reasoningEffort, cwd: channel.displayCwd }), channel.rows.length === 0 && channel.loadedContext !== undefined && (_jsx(LoadedContextPanel, { context: channel.loadedContext, open: loadedContextOpen, onToggle: () => { setLoadedContextOpen(previous => !previous); } })), _jsx(MessageList, { rows: channel.rows, failureHintRowId: failureHintRowId, failureHint: t('traj-hint-failure', { key: `${modLabel}t` }), expanded: expanded, expandedRows: expandedRows, selectedId: selectionActive ? selectedId : null, onToggleRow: toggleRowExpanded, model: channel.model, showAll: showAllMessages, thinkingVisible: thinkingVisible, onToggleAll: () => { setShowAllMessages(previous => !previous); }, onLoadOlder: () => channel.loadOlder(), registerRowRef: registerRowRef, scrollHandle: handle, forceMountRowId: forceMountRowId, newSinceRowId: isSticky ? null : lastSeenRowIdRef.current, onUnseenCount: setUnseenCount })] }), _jsxs(Box, { flexDirection: "column", flexShrink: 0, children: [showPill && (_jsx(NewMessagesPill, { count: unseenCount, onClick: () => handle?.scrollToBottom() })), channel.working &&
