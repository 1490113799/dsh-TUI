 ...
 */
export function Chat({ channel, onExit, }) {
    ...
    const titlePrefix = channel.working
        ...
        : '✦';
    useTerminalTitle(`${titlePrefix} 🐋 ${channel.sessionTitle ?? 'dsh-cc'}`);
    ...
    const selectableRows = channel.rows.filter(row => SELECTABLE_KINDS.has(row.kind));
    ...
    return (_jsxs(Box, { flexDirection: "column", flexGrow: 1, width: "100%", children: [!isSticky && channel.lastUserText && (_jsx(StickyPromptHeader, { text: channel.lastUserText, onClick: () => {
                    ...
                    if (el)
                        ...
                        handle?.scrollToBottom();
                } })), _jsxs(ScrollBox, { ref: setHandle, flexGrow: 1, flexShrink: 1, stickyScroll: true, children: [_jsx(LogoHeader, { model: channel.model, effort: channel.reasoningEffort, cwd: channel.cwd }), _jsx(MessageList, { rows: channel.rows, expanded: expanded, expandedRows: expandedRows, selectedId: selectionActive ? selectedId : null, onToggleRow: toggleRowExpanded, model: channel.model, showAll: showAllMessages, thinkingVisible: thinkingVisible, onToggleAll: () => setShowAllMessages(previous => !previous), registerRowRef: (rowId, el) => {
                            if (el)
                                rowRefsRef.current.set(rowId, el);
                            else
                                rowRefsRef.current.delete(rowId);
                        } })] }), showPill && (_jsx(NewMessagesPill, { count: newCount, onClick: () => handle?.scrollToBottom() })), channel.working && (_jsx(WorkingSpinner, { mode: channel.spinnerMode, hasActiveTools: channel.activeToolCount > 0, responseLengthRef: responseLengthRef, loadingStartTimeRef: loadingStartTimeRef, totalPausedMsRef: totalPausedMsRef, pauseStartTimeRef: pauseStartTimeRef, thinkingStatus: thinkingStatus })), thinkingOpen && (_jsx(ThinkingToggle, { currentValue: thinkingVisible, focusIndex: thinkingFocus, confirmationPending: thinkingConfirm })), resumePickerOpen && resumeSessions.length > 0 && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(ResumePicker, { sessions: resumeSessions, focusIndex: resumeIndex, currentSessionId: channel.agentId }) })), modelPickerOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: models.length === 0 ? (_jsx(ModelPickerLoading, {})) : (_jsx(ModelPicker, { models: models, focusIndex: modelIndex, currentModel: channel.model })) })), historyOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(HistorySearchDialog, { query: historyQuery, cursorOffset: historyCursor, matches: historyMatches, focusIndex: historyFocus }) })), rewindOpen && (_jsx(Box, { flexDirection: "column", marginTop: 1, children: _jsx(RewindPicker, { rows: rewindRows, focusIndex: rewindIndex, confirmRow: rewindConfirm }) })), searchOpen && _jsx(TranscriptSearchBar, { query: searchQuery, cursorOffset: searchCursor, count: searchCount, current: searchCurrent }), _jsx(PromptInput, { channel: channel, helpOpen: helpOpen, onToggleHelp: () => setHelpOpen(previous => !previous), onRunCommand: runCommand, selectionActive: selectionActive || modelPickerOpen || resumePickerOpen || thinkingOpen || historyOpen || rewindOpen || searchOpen, fillText: historyFill, onFillConsumed: () => setHistoryFill(null), onRewindRequest: openRewind }), _jsx(StatusLine, { channel: channel, selectionActive: selectionActive, helpOpen: helpOpen })] }));
}
/**
