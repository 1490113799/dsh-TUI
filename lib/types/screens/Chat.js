...
export function Chat({ channel, questionStore, onExit, }) {
    ...
    useCopyOnSelect(text => channel.notify(`已复制 ${text.length} 个字符`, { timeoutMs: 1500 }));
        ...
        switch (name) {
            ...
            case 'theme': {
                ...
                if (parts.length > 0) {
                    ...
                    channel.notify(ok ? `主题已切换：${parts[0]}（已保存）` : `未知主题「${parts[0]}」· /theme 查看全部`, { color: ok ? 'success' : 'error' });
                    return true;
                }
                setHelpOpen(false);
        ...
        if (themePickerOpen) {
            ...
            if (key.upArrow) {
                ...
                if (name !== undefined) {
                    ...
                    channel.notify(ok ? `主题已切换：${name}（已保存）` : `主题「${name}」切换失败（无法写入 ~/.dsh-cc/theme.json）`, { color: ok ? 'success' : 'error' });
                }
            }
            else if (key.escape) {
                setThemePickerOpen(false);
