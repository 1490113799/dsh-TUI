...
export async function apply(ctx, config) {
    ...
    const configuredDefault = ctx.get('agentDefaultModel')?.currentSelection?.();
    ...
    try {
        ...
        if (!attached) {
            ctx.logger.warn(`dsh-tui: session "${agent.session.id}" has no workspace ownership because workspaceRegistry is not mounted`);
        }
    }
    catch (error) {
        ...
        ctx.logger.warn(`dsh-tui: session "${agent.session.id}" workspace attachment failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    const funnel = createExitFunnel({
        onUserExit: error => {
            ...
            if (error !== undefined) {
                ...
                void finishExit(ctx, instance, config.fullscreen === true, undefined, `dsh-tui crashed: ${message}`, () => disposeRootAndExit(ctx, 1));
                return;
            }
            if (updateRequested) {
                try {
                ...
                void finishExit(ctx, instance, config.fullscreen === true, 'Updating @deepseek-harness-tui/dsh-tui and restarting…', undefined, () => runUpdate(ctx, profile, channel.agentId));
                return;
            }
            ...
            const hint = resumable
                ...
                : undefined;
            void finishExit(ctx, instance, config.fullscreen === true, hint, undefined, () => disposeRootAndExit(ctx, 0));
        },
    });
...
function readInkShutdownState(value) {
    if (value === null || typeof value !== 'object')
        return undefined;
    const candidate = value;
    if (candidate.detachForShutdown !== undefined && typeof candidate.detachForShutdown !== 'function')
        return undefined;
...
function cursorMoveToFrameEnd(runtime) {
...
function runUpdate(ctx, profile, sessionId) {
    disposeRootAndThen(ctx, () => {
        if (profile === undefined) {
            process.stderr.write('\ndsh-tui update aborted: no dsh profile resolved.\n');
            process.exit(1);
        }
        void updateTuiAndRestart(sessionId, profile).then(({ updateCode, restartCode }) => {
            if (updateCode !== 0) {
