 ...
 * color gradient plus a moving highlight window (the same 200ms sweep
 * cadence as the wordmark shimmer). Glyphs are 5 columns wide so curves
 ...
 * `flash` travels left to right (one column per 200ms frame, matching the
 * wordmark's useShimmerAnimation cadence). Returns 5 ANSI rows.
 */
export declare function renderBigText(text: string, time: number, from: Rgb, to: Rgb, flash: Rgb): string[];
