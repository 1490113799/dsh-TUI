...
export function MessageList({ rows, expanded, expandedRows, selectedId, onToggleRow, model, showAll, onToggleAll, thinkingVisible = true, registerRowRef, }) {
    ...
    return (_jsxs(_Fragment, { children: [!showAll && hiddenCount > 0 && (_jsx(Box, { marginTop: 1, onClick: onToggleAll, children: _jsx(Divider, { title: ` ctrl+e to show ${hiddenCount} previous messages ` }) })), visibleRows
                ...
                switch (row.kind) {
                    ...
                    case 'assistant':
                        return row.streaming ? (_jsxs(Box, { alignItems: "flex-start", flexDirection: "row", marginTop: addMargin ? 1 : 0, width: "100%", backgroundColor: rowBackground(row.id), children: [_jsx(Box, { minWidth: 2, children: _jsx(Text, { color: "text", children: "\u25CF" }) }), _jsx(Box, { flexDirection: "column", children: _jsx(StreamingMarkdown, { children: row.text }) })] }, row.id)) : (_jsxs(Box, { width: "100%", flexDirection: "column", backgroundColor: rowBackground(row.id), ref: el => registerRowRef?.(row.id, el), children: [expanded && (_jsx(Box, { flexDirection: "row", justifyContent: "flex-end", gap: 1, marginTop: 1, children: _jsx(MessageMetadata, { timestamp: row.time, model: model }) })), _jsx(AssistantTextMessage, { text: row.text, addMargin: addMargin, isSelected: isSelected, isExpanded: expandedRows.has(row.id), onClick: () => onToggleRow(row.id) })] }, row.id));
                    case 'reasoning':
