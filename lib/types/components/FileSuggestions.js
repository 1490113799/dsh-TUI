...
export function FileSuggestions({ files, selectedIndex, columns, }) {
    ...
    return (_jsx(_Fragment, { children: visible.map(file => {
            ...
            return (_jsxs(Text, { wrap: "truncate", children: [_jsxs(Text, { color: isSelected ? 'suggestion' : undefined, dimColor: !isSelected, children: ["+ ", isDir ? file.slice(0, -1) : file] }), _jsxs(Text, { color: isSelected ? 'suggestion' : undefined, dimColor: !isSelected, children: [' '.repeat(Math.max(1, 20 - file.length)), description.length > descriptionWidth
