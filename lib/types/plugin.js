...
async function resolveAgent(ctx, requestedSessionId, configuredRoute, startupRoute, meta, configuredPreset) {
    ...
    if (rejected !== undefined) {
        ctx.logger.warn(`cc-tui: model route ${rejected.provider}/${rejected.model} is not advertised by provider "${rejected.provider}"; falling back to ${route.provider}/${route.model}`);
    }
    const created = await ctx.agents.create({
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`cc-tui: failed to create agent (provider=${route.provider}, model=${route.model}): ${message}`);
    });
